...
(function () {
  ...
  function renderCommittees() {
    ...
    pane.innerHTML =
      ...
      '<table class="table">' +
        '<thead><tr><th>Acronym</th><th>Name</th><th>Colour</th><th>Logo</th><th>Group Photo</th><th>Members</th><th class="actions-cell">Actions</th></tr></thead>' +
      ...
      state.committees.filter(function (c) { return c.slug !== 'support'; }).map(function (c) {
        ...
        return '<div class="card settings-section" style="margin-top:1.5rem">' +
          '<h3 style="display:flex;align-items:center;gap:.5rem">' +
            '<span style="display:inline-block;width:12px;height:12px;border-radius:50%;background:' + esc(c.color || '#888') + '"></span>' +
          ...
          '<table class="table">' +
            ...
            (members.length ? '' : '<tr><td colspan="4" class="empty">No members yet.</td></tr>') +
            '</tbody>' +
          '</table>' +
        '</div>';
      }).join('') +
    ...
    pane.querySelectorAll('[data-com-add]').forEach(function (b) {
    ...
    pane.querySelectorAll('[data-m-del]').forEach(function (b) {
      b.addEventListener('click', function () { delCommitteeMember(b.getAttribute('data-m-del')); });
    });
  }
  ...
  function committeeMemberModal(m, committeeSlug) {
    m = m || { id: '', committee: committeeSlug || 'scope', name: '', role: '', photo: '', quote: '', sort_order: state.committeeMembers.length };
